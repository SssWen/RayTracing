...
Ptr<HitableList> GenScene() {
	...
	std::shared_ptr<Texture> texture = std::make_shared<Check_Texture>(std::make_shared<Constant_Texture>(Vec3f(0.2, 0.3, 0.1)), std::make_shared<Constant_Texture>(Vec3f(0.9)));
...
Ptr<Hitable> GenRandomScene() {
		...
		for (int b = -11; b < 11; b++) {
			...
			if ((center - Vec3f(4, 0.2, 0)).Norm() > 0.9f) {
				...
				if (choose_mat < 0.8f) // diffuse
					material = DiffuseMaterial::New(std::make_shared<Constant_Texture>(Vec3f(Util::RandF(), Util::RandF(), Util::RandF())));
	...
	balls.push_back(Sphere::New({ -4, 1, 0 }, 1.f, DiffuseMaterial::New(std::make_shared<Constant_Texture>(Vec3f(0.4, 0.2, 0.1)))));
	balls.push_back(Sphere::New({ 4, 1, 0 }, 1.f, MetalMaterial::New({ 0.7, 0.6, 0.5 }, 0.f)));


	std::shared_ptr<Texture> check_texture = std::make_shared<Check_Texture>(std::make_shared<Constant_Texture>(Vec3f(0.2, 0.3, 0.1)), std::make_shared<Constant_Texture>(Vec3f(0.9)));
...
void SaveImg(const vector<vector<Vec3f>> & img) {
	...
	ofstream rst("../../Output/16-PerlinTexture.ppm"); // 层次包围盒
	rst << "P3\n" << width << " " << height << "\n255\n";
