...
int main() {
	...
	auto ground = Sphere::New({ 0, -100.5, -1 }, 100.f, DiffuseMaterial::New(Vec3f(0.8, 0.3, 0.3)));
	// metal
	...
	//auto sphereInerLeft = Sphere::New({ -1, 0, -1 }, -0.5f, DieletricMaterial::New(1.5f));
	auto sphereRight = Sphere::New({ 1, 0, -1 }, 0.5f, MetalMaterial::New(Vec3f(0.8, 0.6, 0.2), 1.0));

	auto scene = HitableList::New({ sphere,sphereLeft,sphereRight,ground });

	ofstream rst("../../Output/10-DielectriMaterial.ppm"); // ppm 是一种简单的图片格式	
